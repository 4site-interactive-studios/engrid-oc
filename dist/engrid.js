 ...
 *
 *  Date: Monday, November 8, 2021 @ 13:21:44 ET
 *  By: bryancasler
...
const tippy = (__webpack_require__(861)/* ["default"] */ .ZP);
...
class app_App extends engrid_ENGrid {
  ...
  onLoad() {
    ...
    if (this.inIframe()) {
      ...
      if (app_App.debug) console.log("iFrame Event - window.onload");
      sendIframeHeight();
      ...
      document.addEventListener("click", e => {
        if (app_App.debug) console.log("iFrame Event - click");
        setTimeout(() => {
          sendIframeHeight();
        }, 100);
      });
  ...
  onValidate() {
    if (this.options.onValidate) {
      if (app_App.debug) console.log("Client onValidate Triggered");
  ...
  onSubmit() {
    if (this.options.onSubmit) {
      if (app_App.debug) console.log("Client onSubmit Triggered");
  ...
  onError() {
    if (this.options.onError) {
      if (app_App.debug) console.log("Client onError Triggered");
  ...
  loadIFrame() {
    if (this.inIframe()) {
      ...
      app_App.setBodyData("embedded", ""); // Fire the resize event

      if (app_App.debug) console.log("iFrame Event - First Resize");
      sendIframeHeight();
    }
...
class SimpleCountrySelect {
  ...
  init() {
    if (this.countrySelect) {
      ...
      if (countrySelectValue.toUpperCase() == "US" || countrySelectValue.toUpperCase() == "USA") {
